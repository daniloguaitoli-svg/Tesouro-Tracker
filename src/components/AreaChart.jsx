...
export function AreaChart({ points, height = 170, pad = 6, color = "var(--accent)", gridLines = 3, campo = "taxa" }) {
  const gid = useId();
  const width = 366;
  const vals = (points ?? [])
    .map((p) => (typeof p === "number" ? p : p[campo]))
  ...
  const line = vals.map((c, i) => `${i ? "L" : "M"}${x(i).toFixed(1)} ${y(c).toFixed(1)}`).join(" ");
